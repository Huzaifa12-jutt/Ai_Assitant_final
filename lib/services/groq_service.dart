...
//  Groq AI Integration — llama3-70b-8192
//
//  Security: Provide the key via:
...
''';
  ...
  ParsedReminder? parseReminder(String text) {
    ...
    // Parse date
    if      (lower.contains('today'))     { when = DateTime(now.year, now.month, now.day, hour, minute); }
    else if (lower.contains('tomorrow'))  { final t = now.add(const Duration(days: 1)); when = DateTime(t.year, t.month, t.day, hour, minute); }
    else if (lower.contains('monday'))    { when = _nextWeekday(now, DateTime.monday,    hour, minute); }
    else if (lower.contains('tuesday'))   { when = _nextWeekday(now, DateTime.tuesday,   hour, minute); }
    else if (lower.contains('wednesday')) { when = _nextWeekday(now, DateTime.wednesday, hour, minute); }
    else if (lower.contains('thursday'))  { when = _nextWeekday(now, DateTime.thursday,  hour, minute); }
    else if (lower.contains('friday'))    { when = _nextWeekday(now, DateTime.friday,    hour, minute); }
    else if (lower.contains('saturday'))  { when = _nextWeekday(now, DateTime.saturday,  hour, minute); }
    else if (lower.contains('sunday'))    { when = _nextWeekday(now, DateTime.sunday,    hour, minute); }
  ...
  Future<String> chat(String userMessage, {
    ...
    if (_apiKey.isEmpty) {
      return '🔑 Groq API key missing. Run with --dart-define=GROQ_API_KEY=YOUR_KEY';
    }
    try {
      ...
      if (e.response?.statusCode == 401) {
        return '🔑 Invalid Groq API key (401). Run the app with: --dart-define=GROQ_API_KEY=YOUR_KEY';
      }
      return _fallbackResponse(userMessage);
    } catch (_) {
      return _fallbackResponse(userMessage);
...
// ── Chat Intent ──────────────────────────────────────────────────────────
enum ChatIntent {
  emailSummary, draftReply, searchEmails, sendEmail,
  viewReminders, setReminder,
  dayBrief, calendarView,
